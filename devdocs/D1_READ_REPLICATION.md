...
`ai-broker-db` runs its primary instance in ENAM. With read replication turned
...
`apps/ai-broker-web/lib/db/d1-session.ts` owns this. Per request:
...
2. The `DB` binding is wrapped by `sessionedD1()`, so every statement drizzle
   prepares runs inside that one session. The wrapped binding is also published
   on `globalThis` as `__D1_SESSION_DB__`, so `packages/investing` — which reads
   its bindings from there rather than importing `cloudflare:workers` — joins
   the same session instead of opening a second, unsessioned read path.
...
- Cloudflare dashboard → D1 → `ai-broker-db` → Settings → Enable Read Replication, or
- `apps/ai-broker-web/scripts/d1-read-replication.sh enable`, with
  `CLOUDFLARE_ACCOUNT_ID` and a `CLOUDFLARE_API_TOKEN` holding `D1:Edit`.

`apps/ai-broker-web/scripts/d1-read-replication.sh status` prints the
database's current `read_replication.mode` (`auto` = on, `disabled` = off).
