 ...
 * nearest the request instead of from the single primary instance in ENAM —
 ...
 *      drizzle prepares runs inside that request's single session. The Worker
 *      entry also publishes the wrapped binding on `globalThis` so
 *      `packages/investing`, which reads its bindings from there rather than
 *      importing `cloudflare:workers`, joins the same session.
...
/**
 * Open a session scope outside of a request — the cron handlers, which have no
