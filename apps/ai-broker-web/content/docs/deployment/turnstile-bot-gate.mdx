...
A desktop browser's **first** HTML page view of AI Broker is answered with a
...
The whole feature lives in `apps/ai-broker-web/lib/turnstile/` and is called from one
place — the top of the Worker's `fetch` in `apps/ai-broker-web/worker/index.ts`:
...
1. Create a **Managed** widget in the Cloudflare dashboard under **Turnstile**,
   with `autoinvestment.broker` (and any other hostname the app answers on) in its
   allowed list.
...
| `TURNSTILE_COOKIE_DOMAIN` | no | e.g. `.autoinvestment.broker` to share one pass across subdomains. |
...
`apps/ai-broker-web/lib/turnstile/__tests__/turnstile.test.ts` covers the decision
