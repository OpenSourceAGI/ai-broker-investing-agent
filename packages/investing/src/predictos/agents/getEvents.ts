/**
 * Get events agent (ported/adapted from PredictOS `get-events`).
 *
...
/**
 * Extract event data (markets) from a prediction market URL.
 */
export async function getEvents(
  ...
  try {
    ...
    if (!pmType || !urlSource) {
      return {
        success: false,
        error: "Could not detect prediction market type from URL. Use Kalshi, Polymarket, or Jupiter prediction market URLs.",
      };
    }
