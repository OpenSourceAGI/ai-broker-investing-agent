/**
 * Mapper agent (ported/adapted from PredictOS `mapper-agent`).
 *
...
/**
 * Map an analysis result into platform-specific order parameters.
 */
export async function mapperAgent(
  input: MapperAgentRequest,
): Promise<MapperAgentResponse> {
  const startTime = Date.now();

  const { platform, analysisResult, marketData, budgetUsd } = input;
